...
(function ($) {
  $.fn.dropdown = function (settings) {
        ...
        $(this).on('click', function () {
          if (target.is(':hidden')) {
            changeAction(
              ...
              $(this)
                .siblings()
                .find(settings.targetElement + ':not(:hidden)'),
              aspect
            );
            changeAction('show', target, aspect);
          } else {
    ...
    if (settings.animate === 'direct') {
      ...
      $ddElmt.find(settings.targetElement).animate(startOp, 0, 'linear', function () {
        $(this).hide();
      });
    }
